...
function Calendar({ loggedUser, startCity, destinationCity, setStartCity, setDestinationCity }) {
  ...
  const startCityCode = startCity.split('-')[1]?.trim(); // Extracting the code part after the '-'
  const destinationCityCode = destinationCity.split('-')[1]?.trim(); // Extracting the code part after the '-'

  const timeSlots = [
    `${startCityCode} - ${destinationCityCode} SF Express`,
    `${startCityCode} - ${destinationCityCode} SPL SF Express`,
    `${startCityCode} - ${destinationCityCode} Express`,
    `${startCityCode} - ${destinationCityCode} DEMU`,
    `${startCityCode} - ${destinationCityCode} Passenger`,
    `${startCityCode} - ${destinationCityCode} SPL FARE SPL`,
  ];
...
const getAvailableSlotsCount = (year, month, date) => {
  ...
  const handleBooking = () => {
    ...
    if (selectedDate !== null && selectedSlot !== null) {
      const bookingData = {
        ...
        seats: selectedSeats // Include the number of seats
  ...
  const getBookedSeatsCount = async (year, month, date, slot) => {
    try {
      // Fetch the booked slots data from the backend
      const response = await fetch(`https://oneshot-ai-backend.onrender.com/get-booking`);
      const data = await response.json();
      console.log(year, month, date, slot);
      // Filter bookings with the same year, month, date, and slot
      const filteredBookings = data.payload.filter(booking => (
        booking.year === year &&
        booking.month === month &&
        booking.date === date &&
        booking.slot === slot // Check if seats property exists and is a number
      ));
  
      // Calculate the total number of seats booked for the specified slot
      ...
      <>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Select a Train on {expandedDate}/{selectedMonth + 1}/{selectedYear}</Modal.Title>
          </Modal.Header>
          <Modal.Body>
          ...
          <Modal.Footer>
            ...
            {selectedSlot !== null && (
            <div>
              <select value={selectedSeats} onChange={(e) => setSelectedSeats(parseInt(e.target.value))}>
                {[1, 2, 3, 4, 5].map((num) => (
                  ...
                  <div className="date-cell">
                    <div
                      ...
                      className={`circle
                        ...
                        ${isDatePartiallyBooked(selectedYear, selectedMonth + 1, day + 1) && !isDateDisabled(selectedYear, selectedMonth, day + 1) ? '' : ''}
                      `}
